...
import { getRecommendedToursForUser } from "../../../../api/user/ApiRecommendedToursForUser";

function RecommendTours({ userId, onclickChangePage }) {
  ...
  const CardInfor = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getRecommendedToursForUser(userId);
